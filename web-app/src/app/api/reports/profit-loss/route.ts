...
export async function GET(request: Request) {
  try {
    ...
    const sales = await prisma.sale.findMany({
      where: salesWhere,
      include: { sale_items: true, sale_other_credits: true }
    });
    ...
    sales.forEach(sale => {
      ...
      // 5b. Include Other Credits (e.g. Installation Charges, Services) as 100% Gross Profit
      const shouldIncludeCredits = !biltiParam || sale.sale_items.some(it => it.bilti_no === biltiParam);
      if (shouldIncludeCredits && sale.sale_other_credits && sale.sale_other_credits.length > 0) {
        sale.sale_other_credits.forEach(credit => {
          const creditAmount = Number(credit.amount) || 0;
          if (creditAmount <= 0) return;

          const creditQty = Number(credit.quantity) || 1;
          const creditRate = Number(credit.rate) || creditAmount;

          grossTradingProfit += creditAmount;

          const key = `credit|${credit.item_name}|${creditRate}`;
          const existing = tradingItems.find(t => t.key === key);
          if (existing) {
            existing.qtySold += creditQty;
            existing.totalSaleValue += creditAmount;
            existing.profit += creditAmount;
          } else {
            tradingItems.push({
              key,
              itemName: `${credit.item_name} (Installation / Service)`,
              powerWatt: null,
              biltiNo: biltiParam ? biltiParam : null,
              qtySold: creditQty,
              purchaseRate: 0,
              sellRate: creditRate,
              totalSaleValue: creditAmount,
              profit: creditAmount
            });
          }
        });
