...
export async function getAvailableStock(): Promise<ActionResult<Array<{
  ...
  try {
    const [purchases, saleItems, catalogProducts] = await Promise.all([
    ...
    // Ensure catalog products with 0 purchases appear in general stock
    catalogProducts.forEach(prod => {
      if (!prod.item_name) return;
      const name = prod.item_name;
      const key = `${name.toLowerCase().trim()}::none::none`;
      if (!stockMap[key]) {
        stockMap[key] = { item_name: name, power_watt: null, bilti_no: null, purchased: 0, sold: 0 };
      }
    });
