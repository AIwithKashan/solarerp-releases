      ...
      {open && (
        <ul ref={listRef} className="combobox-dropdown" role="listbox">
          {filtered.map((s, idx) => {
            ...
            return (
              <li key={`${s.item_name}::${s.power_watt ?? 'none'}::${s.bilti_no ?? 'none'}::${idx}`} data-cbitem
                className={cls('combobox-item', isSel && 'selected', cursor === idx && 'highlighted')}
                onClick={() => select(s)}
                onMouseEnter={() => setCursor(idx)}
                style={{ opacity: isOut ? 0.75 : 1 }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', gap: '8px', alignItems: 'center' }}>
                  ...
                  {s.available > 0 ? (
                    <span style={{ fontSize: '0.8rem', color: 'var(--c-primary)', fontWeight: 600 }}>
                      Stock: {s.available}
                    </span>
                  ) : (
                    <span style={{ fontSize: '0.72rem', color: '#ef4444', background: '#fef2f2', border: '1px solid #fecaca', padding: '1px 6px', borderRadius: '4px', fontWeight: 700 }}>
                      Out of Stock
                    </span>
                  )}
                </div>
          ...
          {filtered.length === 0 && (
            <li className="combobox-empty">
              {stockItems.length === 0 ? 'No items found in selected Bilti' : 'No stock items matched'}
            </li>
          )}
        </ul>
      )}
...
function SaleForm({
  ...
  // Dynamic stock availability calculation that respects specific Bilti batches
  const getMaxAvailable = (item: SaleItemInsert): number => {
    if (!item.item_name) return 0;
    const name = item.item_name.toLowerCase().trim();
    const watt = item.power_watt ?? null;
    const bilti = item.bilti_no ? item.bilti_no.trim() : null;

    let total = 0;
    stockItems.forEach(s => {
      if (s.item_name.toLowerCase().trim() === name && (s.power_watt ?? null) === watt) {
        if (bilti) {
          if ((s.bilti_no || '').trim() === bilti) {
            total += s.available;
          }
        } else {
          total += s.available;
    ...
    if (editTarget && editTarget.sale_items) {
      editTarget.sale_items.forEach(eit => {
        if (eit.item_name.toLowerCase().trim() === name && (eit.power_watt ?? null) === watt) {
          if (bilti) {
            if ((eit.bilti_no || '').trim() === bilti) {
              total += eit.quantity;
            }
          } else {
            total += eit.quantity;
  ...
  // Helper to get stock items filtered for a specific row
  const getRowStockItems = (item: SaleItemInsert) => {
    if (item.bilti_no && item.bilti_no.trim()) {
      // STRICT FILTER: Only products purchased under this specific Bilti!
      return stockItems.filter(s => (s.bilti_no || '').trim() === item.bilti_no!.trim());
    }

    // Aggregated list: each unique product variant shown once with summed stock
    const map: Record<string, typeof stockItems[0]> = {};
    stockItems.forEach(s => {
      const key = `${s.item_name.toLowerCase().trim()}::${s.power_watt ?? 'none'}`;
      if (!map[key]) {
        map[key] = { ...s, bilti_no: null };
      } else {
        map[key] = {
          ...map[key],
          available: map[key].available + s.available
        };
      }
    });
  ...
  // Helper when changing Bilti on an item
  const handleBiltiChange = (index: number, newBilti: string | null) => {
    setItems(prev => prev.map((item, i) => {
      if (i !== index) return item;
      const biltiVal = newBilti ? newBilti.trim() : null;

      // Check if current item exists in the newly selected Bilti
      let keepItem = false;
      let newWatt = item.power_watt;
      if (item.item_name && biltiVal) {
        const match = stockItems.find(s => 
          s.item_name.toLowerCase().trim() === item.item_name.toLowerCase().trim() &&
          (s.bilti_no || '').trim() === biltiVal
        );
        if (match) {
          keepItem = true;
          newWatt = match.power_watt;
        }
      } else if (item.item_name && !biltiVal) {
        keepItem = true;
      }
      ...
      const updated = {
        ...item,
        bilti_no: biltiVal,
        item_name: keepItem ? item.item_name : '',
        power_watt: keepItem ? newWatt : null,
        accounting_unit: keepItem ? item.accounting_unit : 'Number',
        quantity: keepItem ? Math.max(1, item.quantity) : 1,
        amount: keepItem ? item.amount : 0
      };
  ...
  const validate = (): boolean => {
    ...
    items.forEach((it, idx) => {
      ...
      if (it.quantity <= 0) {
        errs[`item_${idx}_qty`] = 'Qty must be > 0.';
        return;
      }
      const available = getMaxAvailable(it);
      if (available <= 0) {
        errs[`item_${idx}_qty`] = `"${it.item_name}" is Out of Stock.`;
      ...
      <div className="account-table-wrap" style={{ overflow: 'visible', padding: '20px', background: 'var(--c-bg-card)', border: '1px solid var(--c-border)', borderRadius: '8px' }}>
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-4 mb-4">
            {items.map((it, idx) => {
              const maxAvailable = getMaxAvailable(it);
              return (
                <div key={idx} className="form-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '12px', alignItems: 'flex-end', padding: '16px', border: '1px dashed var(--c-border)', borderRadius: '8px', background: 'var(--c-bg-alt)', borderColor: 'var(--c-border)', zIndex: 80 - idx, position: 'relative' }}>
                  ...
                  {/* 1. Bilti No / Container # Dropdown (First Field) */}
                  <div className="field-group" style={{ zIndex: 105, position: 'relative' }}>
                    <label className="field-label">Bilti / Batch</label>
                    <select
                      className="field-input"
                      value={it.bilti_no || ''}
                      onChange={e => handleBiltiChange(idx, e.target.value || null)}
                    >
                      <option value="">-- All / General Stock --</option>
                      {purchaseBiltis.length > 0 && (
                        <optgroup label="Purchase Biltis / Batches">
                          {purchaseBiltis.map(b => {
                            const biltiStock = stockItems
                              .filter(s => (s.bilti_no || '').trim() === b.trim())
                              .reduce((sum, cur) => sum + cur.available, 0);
                            return (
                              <option key={b} value={b}>
                                {b} {biltiStock > 0 ? `(${biltiStock} in stock)` : '(Out of Stock)'}
                              </option>
                            );
                          })}
                        </optgroup>
                      )}
                      {it.bilti_no && !purchaseBiltis.includes(it.bilti_no) && (
                  ...
                  {/* 2. Item Selection Combobox (Filtered strictly to selected Bilti) */}
                  <div className="md:col-span-2 field-group" style={{ zIndex: 100, position: 'relative' }}>
                    ...
                    <ProductStockCombobox
                      ...
                      onChange={(val, watt, bilti) => {
                        ...
                        setItems(prev => prev.map((item, i) => {
                          ...
                          const updated = {
                            ...
                            bilti_no: item.bilti_no || bilti || null,
                            quantity: 1,
                          };
                      ...
                      stockItems={getRowStockItems(it)}
                      placeholder={it.bilti_no ? `Select item from ${it.bilti_no}…` : "Select item variant…"}
                  ...
                  {/* 4. Power Rating (Watts - Read-only and Locked) */}
                  <div className="field-group">
                  ...
                  {/* 5. Quantity (Capped at available stock) */}
                  <div className="field-group">
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        if (maxAvailable > 0 && q > maxAvailable) {
                          q = maxAvailable;
                        }
                    ...
                    {it.item_name && (
                      <span style={{ fontSize: '0.72rem', display: 'block', marginTop: '2px' }}>
                        {maxAvailable <= 0 ? (
                          ...
                          <span style={{ color: 'var(--c-text-subtle)' }}>Stock: {maxAvailable}</span>
                        )}
                      </span>
                    )}
