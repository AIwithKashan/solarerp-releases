  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      <div className="account-form" style={{ borderTop: '3px solid #6366f1' }}>
        ...
        {/* Dynamic Lines: Desktop Table (>= 641px) */}
        <div className="jv-lines-desktop table-scroll" style={{ marginBottom: 20, overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
          <table className="data-table table-keep" style={{ borderCollapse: 'separate', borderSpacing: '0 8px', minWidth: 620 }}>
            <thead>
              <tr>
        ...
        {/* Dynamic Lines: Mobile Touch Cards (<= 640px) */}
        <div className="jv-lines-mobile">
          {lines.map((line, idx) => (
            <div key={idx} className="jv-card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--c-text-muted)', textTransform: 'uppercase' }}>
                  Posting Line #{idx + 1}
                </span>
                <button
                  type="button"
                  ...
                  onClick={() => removeLine(idx)}
                  disabled={isPending || lines.length <= 2}
                  style={{ opacity: lines.length <= 2 ? 0.4 : 1, cursor: lines.length <= 2 ? 'not-allowed' : 'pointer', padding: 4 }}
                  title="Remove line"
                >
                  <Trash2 size={15} />
                </button>
              ...
              <div className="field-group">
                <label className="field-label">Account Name <span className="required">*</span></label>
                <JournalAccountCombobox
                  accounts={accounts}
                  valueId={line.account_id}
                  onChange={(id, name) => {
              ...
              <div className="field-group">
                <label className="field-label">Line Remarks / Narration</label>
                <input
                  className="field-input"
                  value={line.remarks}
                  placeholder="Optional narration…"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div className="field-group">
                  <label className="field-label" style={{ color: '#6366f1' }}>Debit (IN)</label>
                  <input
                    type="number"
                    step="any"
                    min="0"
                ...
                <div className="field-group">
                  <label className="field-label" style={{ color: '#ef4444' }}>Credit (OUT)</label>
                  <input
                    type="number"
                    step="any"
                    min="0"
