...
# Direct Whitelisted Cleanup Targets
target_folders = [
...
print("============================================================")
print("  MASTER C DRIVE SAFE CLEANER V3 - STRICT OMNIROUTE SAFETY ")
print("============================================================")
...
for target in target_folders:
    # Strict OmniRoute Protection Bypass
    if "omniroute" in target.lower() or ".omniroute" in target.lower():
        print(f"[!] OMNIROUTE PROTECTED TARGET SKIPPED: {target}")
        continue
...
# Local node_modules (excluding omniroute)
local_mod = os.path.join(user, ".local", "node_modules")
if os.path.exists(local_mod):
    print("\n[+] Cleaning non-omniroute modules in .local...")
    mod_freed = 0
    mod_deleted = 0
...
print("\n" + "="*60)
print("MASTER SAFE C DRIVE CLEANUP V3 COMPLETE!")
